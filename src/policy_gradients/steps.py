    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for selected in splits:
                ...
                # Total loss, is the min of clipped and unclipped reward for each state, averaged.
                surrogate = (-ch.min(unclp_rew, clp_rew)).mean()   # policy Loss
                entropy = -params.ENTROPY_COEFF * entropy_bonus
                loss = surrogate + entropy
...
def get_state_kl_bound_lipschitz(net, batch_states, batch_action_means, eps, steps, stdev, not_dones=None):
    """ Computing an estimated upper bound of KL divergence using Lipschitz"""
    if not_dones is not None:
        # If we have not_dones, the underlying network is a LSTM.
        wrapped_net = functools.partial(net, not_dones=not_dones)
    else:
    ...
    layers = []
    for layer in wrapped_net.affine_layers:
        layers.append(layer)
        layers.append(wrapped_net.activation)
    layers.append(wrapped_net.final_mean)
    ...
    total = 0
    origin, changed = batch_states.detach(), var_states.detach()
    for index in range(len(model) - 1):
        layer = model[index]
        activation = model[index + 1]
        origin, changed = layer(origin), layer(changed)
        diff = (activation(origin) - activation(changed))
        total += (diff * diff).sum(axis=-1, keepdim=True)
    return total / stdev
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for selected in splits:
                ...
                dist = net(batch_states)
                # Convert state distribution to log likelyhood.
                new_log_ps = net.get_loglikelihood(dist, batch_actions)  # obtain the distance of new Strategy of old Strategy
...
def robust_ppo_step_lip(all_states, actions, old_log_ps, rewards, returns, not_dones,
                    advs, net, params, store, opt_step, relaxed_net, eps_scheduler, beta_scheduler):
    '''
    Proximal Policy Optimization with robustness regularizer
    Runs K epochs of PPO as in https://arxiv.org/abs/1707.06347
    ...
    if params.HISTORY_LENGTH > 0:
        ...
        # We normalize all advantages at once instead of batch by batch, since each batch may contain different number of samples.
        normalized_advs = adv_normalize(advs)
        batches, alive_masks, time_masks, lengths = pack_history([all_states, actions, old_log_ps, normalized_advs],
                                                                 not_dones, max_length=params.HISTORY_LENGTH)
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for i, batch in enumerate(batches):
                ...
                # dimension is the same as minibatch size.
                # We already normalized advs before. No need to normalize here.
                unclp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps, mask=mask,
                                             normalize=False)
                clp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps,
                                           clip_eps=params.CLIP_EPS, mask=mask, normalize=False)
            ...
            for selected in splits:
                ...
                dist = net(batch_states)
                # Convert state distribution to log likelyhood.
                new_log_ps = net.get_loglikelihood(dist,
                                                   batch_actions)  # obtain the distance of new Strategy of old Strategy
                ...
                if params.ROBUST_PPO_METHOD == "convex-relax":
                    ...
                    kl_upper_bound = get_state_kl_bound_sgld(net, batch_states, batch_action_means,
                                                             eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                                                             stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "lipschitz":
                    # compute an upper bound.
                    kl_upper_bound = get_state_kl_bound_lipschitz(net, batch_states, batch_action_means,
                                                             eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                                                             stdev=stdev).mean()
                else:
                    raise ValueError(f"Unsupported robust PPO method {params.ROBUST_PPO_METHOD}")
...
def step_with_mode(mode, adversary=False):
    STEPS = {
        ...
        # add the Lipschitz
        'robust_ppo_lip': robust_ppo_step_lip,
        'adv_ppo': ppo_step,
        'adv_trpo': trpo_step,
