...
class net_Local_Lip(nn.Module):
    ...
    def forward(self, x, epsilon=None, u_list=None, u_data=None, sniter=1):
        ...
        # Dn_mask: indicator for upper bound smaller than zero 上限小于0的指标
    ...
    def local_linear(self):
        ...
        for i in range(depth - 1):
            ...
            if not isinstance(self.model[i], nn.ReLU):
                layer = self.model[i]
                W = layer.weight
                ...
                for itr in range(self.sniter):
                    ...
                    #if 'relu' in self.args.model: # **** args.model
                ...
                # specnorms[i] = sigma
                specnorms.append(sigma)

        # remove -1.0 elements in specnorms (correspond to relu or flatten layer)
        specnorms = [x for x in specnorms if
                     (torch.is_tensor(x) == True or (torch.is_tensor(x) == False and x != -1.0))]
...
def BCP_translation(mu_prev, r_prev, ibp_mu_prev, ibp_r_prev, W, opt_iter, show):
    ...
    while (iteration == 0 or (
            (diag_zero * wi_wj1 < lb) + (diag_zero * wi_wj1 > ub)).sum() > 0 and iteration < opt_iter):
        iteration += 1
        before_clipped = diag_zero * wi_wj1 * (wi_wj1 <= lb).type(torch.float) + diag_zero * wi_wj1 * (
                    wi_wj1 >= ub).type(torch.float)
        after_clipped = diag_zero * lb * (wi_wj1 <= lb).type(torch.float) + diag_zero * ub * (wi_wj1 >= ub).type(
            torch.float)
