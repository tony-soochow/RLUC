...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        # Create the convex relaxation model
        if self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_ppo_lip':
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        for t in iterator:
            ...
            # for train the strongest adversary
            ...
            if collect_adversary_trajectory:
                ...
                if collect_perturbed_state:
                    ...
                    pairs = [
                        ...
                        (not_dones, next_not_dones),
                        (actions, next_actions), # The sampled actions.
                        (action_means, next_action_meansnext_action_means), # The Gaussian mean of actions.
                        # (action_stds, next_action_stds), # The Gaussian std of actions, is a constant, no need to save.
                        (action_log_probs, next_action_log_probs),
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        if adversary_step:
            ...
            if self.ANNEAL_LR:
                # policy and value Loss scheduler ???
        ...
        # ***** add the robust Lip
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_ppo_lip') and isinstance(self.policy_model, CtsPolicy) and not adversary_step:
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        # if the trainer is only for Adversary
        table_name_suffix = "_adv" if adversary_step else ""

        if adversary_step:
            # For training the best Adversary
            print('++++++++ Adversary training ++++++++++')
            policy_model = self.adversary_policy_model
        else:
            # For Training the SA_PPO Agent
            print('++++++++ Policy training ++++++++++')
            policy_model = self.policy_model

        num_saps = self.T * self.NUM_ACTORS

        # One roll of trajectory
    ...
    def eval_model(self, max_len=2048, mass=1, friction=1):
        print("-" * 80)
        with torch.no_grad():
            output = self.test_evaluation(max_len=max_len, mass=mass, friction=friction)
            ep_length, ep_reward, actions, action_means, states = output
            msg = "Episode reward: %f | episode length: %f"
            print(msg % (ep_reward, ep_length))
        return ep_length, ep_reward

    def test_evaluation(self, max_len, mass=1, friction=1, should_tqdm=False):
        envs = self.envs
        for env in envs:
            # print(env.env.env.model.body_mass)
            # print(env.env.env.model.geom_friction)
            env.env.env.model.body_mass[:] = mass * env.env.env.model.body_mass[:]
            env.env.env.model.geom_friction[:] = friction * env.env.env.model.geom_friction[:]
            # print(env.env.env.model.body_mass)
            # print(env.env.env.model.geom_friction)
        ...
        for t in iterator:
            ...
            if self.params.ATTACK_METHOD != "none":
                ...
                attack_eps = float(self.params.ROBUST_PPO_EPS) if self.params.ATTACK_EPS == "same" else float(
                    self.params.ATTACK_EPS)
                if max_eps > attack_eps + 1e-5:
                    raise RuntimeError(
                        f"{max_eps} > {attack_eps}. Attack implementation has bug and eps is not correctly handled.")
            next_actions = self.policy_model.sample(action_pds)
